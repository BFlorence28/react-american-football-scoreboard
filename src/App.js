//TODO: STEP 1 - Import the useState hook.
import React, {useState} from "react";
import "./App.css";
import BottomRow from "./BottomRow";

function App() {
  //TODO: STEP 2 - Establish your applictaion's state with some useState hooks.  You'll need one for the home score and another for the away score.

  const [homeScore, setHomeScore] = useState(0);
  const [awayScore, setAwayScore] = useState(0);

  const scoreTouchdown = e => {
    setHomeScore(homeScore + 7);
  };

  const scoreFieldGoal = e => {
    setHomeScore(homeScore + 3);
  };

  const scoreAwayTouchdown = e => {
    setAwayScore(awayScore + 7);
  };

  const scoreAwayFieldGoal = e => {
    setAwayScore(awayScore + 3);
  };

  // function homeTeam(scored){
  //   if(scored ==='touchdown'){
  //     return setHomeScore(homeScore + 6)
  //   } else if(scored === 'fieldgoal'){
  //     return setHomeScore(homeScore + 3)
  //   }
  // }

  return (
    <div className="container">
      <section className="scoreboard">
        <div className="topRow">
          <div className="home">
            <h2 className="home__name">Chiefs</h2>

            {/* TODO STEP 3 - We need to change the hardcoded values in these divs to accept dynamic values from our state. */}

            <div className="home__score">{homeScore}</div>
          </div>
          <div className="timer">00:00</div>
          <div className="away">
            <h2 className="away__name">49ers</h2>
            <div className="away__score">{awayScore}</div>
          </div>
        </div>
        <BottomRow />
      </section>
      <section className="buttons">
        <div className="homeButtons">
          {/* TODO STEP 4 - Now we need to attach our state setter functions to click listeners. */}
          <button className="homeButtons__touchdown" onClick={scoreTouchdown}>Home Touchdown</button>
              {/* <button onClick={() => setCount(count + 1)}>Add 1</button> */}

          <button className="homeButtons__fieldGoal" onClick={scoreFieldGoal}>Home Field Goal</button>
        </div>
        <div className="awayButtons">
          <button className="awayButtons__touchdown" onClick={scoreAwayTouchdown}>Away Touchdown</button>
          <button className="awayButtons__fieldGoal" onClick={scoreAwayFieldGoal}>Away Field Goal</button>
        </div>
      </section>
    </div>
  );
}

export default App;
